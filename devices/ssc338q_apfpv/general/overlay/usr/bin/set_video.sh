...
<center><body>---<::: RC32 aalink4s :::>---</body></center>
...
<!-- Tabs -->
<div class="tabs">
  <div class="tab-btn active"        id="tabSetup">Setup</div>
  <div class="tab-btn" id="tabControl">Control</div>
  <div class="tab-btn" id="tabApAlink" style="display:none">Advanced</div>
</div>

<!-- Control Tab Content -->
<div class="tab-content hidden" id="contentControl">
...
<!-- Setup Tab Content -->
<div class="tab-content" id="contentSetup">
  <!-- SSID -->
  <div class="row">
...
<div class="row slider-container">
  ...
  <!-- Link -->
  <div class="row">
    <button class="btn" onclick="goArg('set_osd', linkSel.value)">Set&nbsp;Mode</button>
    <select id="linkSel">
      <option value="mode-manual">Android</option>
      <option value="mode-aalink">VRX Groundstation</option>
    </select>
  </div>
...
<div class="row">
  ...
  <!-- Section heading ---------------------------------------------------- -->
  <div class="section-title">Advanced AAlink Settings</div>

  <!-- HT Mode dropdown + button -->
<div class="row">
  <button class="btn" id="htModeBtn">Apply HT Mode</button>
  <select id="htModeSel">
    <option value="set_wipeht">Wipe HT (default)</option>
    <option value="set_noht">No HT (disallow optional HT params)</option>
    <option value="set_ht40">HT40 (EU2 specific)</option>
    <option value="set_stbc">Allow STBC</option>
    <option value="set_mcs0">Set MCS0 (0x0C)</option>
    <option value="set_mcsff">Set Radio control (0xFF)</option>
  </select>
</div>
...
<div class="row" style="flex-direction:column;">
  <label style="font-size:1em;font-weight:600;text-align:center;margin:.5em 0;">
    Link Status Monitor
  </label>

  <button class="btn info" id="toggleAlinkStatusBtn" style="width:100%;flex:1">
    Show Live Status
  </button>

  <iframe
    id="alinkIframe"
    style="width:100%; min-height:600px; border:1px solid #339;
           border-radius:6px; margin-top:.8em; display:none;">
  </iframe>
</div>
...
document.getElementById('alinkIframe').src = `http://${location.hostname}:8081/`;

const statusBtn   = document.getElementById('toggleAlinkStatusBtn');
const statusFrame = document.getElementById('alinkIframe');

statusBtn.addEventListener('click', () => {
  const showing = statusFrame.style.display === 'block';

  if (showing) {
    // Hide the iframe
    statusFrame.style.display = 'none';
    statusBtn.textContent = 'Show Live Status';
    // Optional: stop polling if you modify the iframe page
    statusFrame.src = '';  // unload to stop polling if needed
  } else {
    // Show and load iframe
    statusFrame.src = `http://${location.hostname}:8081/`;
    statusFrame.style.display = 'block';
    statusBtn.textContent = 'Hide Live Status';
  }
});
...
/* ---------- HT MODE DROPDOWN ---------------- */
const htModeBtn = document.getElementById('htModeBtn');
const htModeSel = document.getElementById('htModeSel');

htModeBtn.addEventListener('click', () => {
  // grab the selected command name
  const cmd = htModeSel.value;   // "set_noht" or "set_ht40"
  // call it directly (no extra args)
  go(cmd);
});
...
// ───── UI ENHANCEMENTS: hide/show Advanced + fetch defaults ─────

// grab all the bits we need
const tabApAlinkBtn     = document.getElementById('tabApAlink');
const tabControlBtn     = document.getElementById('tabControl');
const contentControlPane= document.getElementById('contentControl');
const ssidInput         = document.getElementById('ssid');
const passInput         = document.getElementById('pass');
const ttySel            = document.getElementById('ttySel');
const linkSel           = document.getElementById('linkSel');

// ─── hide/show “Advanced” without breaking its centering ───
tabApAlinkBtn.style.display = 'none';

powerToggle.addEventListener('change', () => {
  if (powerToggle.checked) {
    // reveal, but let CSS handle centering
    tabApAlinkBtn.style.removeProperty('display');
  } else {
    // hide
    tabApAlinkBtn.style.display = 'none';
    // if it was active, bounce back to Control
    if (tabApAlinkBtn.classList.contains('active')) {
      showTab(tabControlBtn, contentControlPane);
    }
  }
});

// ─── simplified, reliable default-value loader ───
function initDefault(el, endpoint) {
  fetchJSONOrText(endpoint, {}, 1000)
    .then(r => {
      if (!r.ok) throw new Error('HTTP ' + r.status);
      return r.text();
    ...
    .then(raw => {
      ...
      if (el.tagName === 'SELECT') {
        // grab the “mode” prefix from your options
        const prefix = el.options[0].value.split('-')[0];

        // for tty values, strip “/dev/” and convert “ttyS1”→“tty1”
        const suffix = v.startsWith('/dev/')
          ? v.slice(5).replace(/^ttyS/, 'tty')
          : v;

        const candidate = `${prefix}-${suffix}`;
        if (el.querySelector(`option[value="${candidate}"]`)) {
          el.value = candidate;
        } else {
          console.warn(`initDefault: no <option> for "${candidate}"`);
        }
      } else {
        // plain <input>
        el.value = v;
    ...
    .catch(err => {
      console.error(`initDefault(${endpoint}) failed:`, err);
    });
}

// ─── fire them all off ───
initDefault(ssidInput, '/value/ssid');
initDefault(passInput, '/value/passw');
initDefault(ttySel,    '/value/msposd_tty');
initDefault(linkSel,   '/value/mode');



// ─── Video Modes dropdown ─────────────────────────────────────────────
const videoSel = document.getElementById('videoSel');

function initVideoModes(attempts = 3) {
  let tries = 0;
  const attempt = () => {
    fetchJSONOrText('/value/video_modes', {}, 1000)
      .then(r => r.ok ? r.json() : Promise.reject('HTTP ' + r.status))
      .then(data => {
        const modes = Array.isArray(data.available_modes)
                    ? data.available_modes
                    : [];
        if (modes.length) {
          // clear old and populate
          videoSel.innerHTML = '';
          modes.forEach(mode => {
            const opt = document.createElement('option');
            // backend wants hyphens
            opt.value       = mode.replace(/ /g, '-');
            opt.textContent = mode;
            videoSel.appendChild(opt);
          });
        } else {
          throw new Error('no modes returned');
        }
      })
      .catch(err => {
        console.error('initVideoModes failed:', err);
        // fallback single un-executable option
        videoSel.innerHTML = '<option value="unknown">unknown</option>';
      });
  };

  // in case of transient failures, retry a couple times
  const wrapped = () => {
    attempt();
    if (++tries < attempts) {
      setTimeout(wrapped, 400 * tries);
    }
  };
  wrapped();
}
...
// ─── auto‐fetch default WLAN channel (robust “ch149-20 --make-permanent” etc.) ───
fetchJSONOrText('/value/wlanchan', {}, 1000)
  .then(r => r.ok ? r.text() : Promise.reject('HTTP ' + r.status))
  .then(raw => {
    const chan     = raw.trim();                   // e.g. "149"
    const candidate= `ch${chan}-20`;               // → "ch149-20"

    // find an <option> whose value starts with that candidate
    const opt = Array.from(chSel.options).find(o =>
      o.value.split(' ')[0] === candidate
    );

    if (opt) {
      // set the full value (including any "--make-permanent" suffix)
      chSel.value = opt.value;
    } else {
      console.warn(`No channel option matching "${candidate}"`);
    }
  })
  .catch(err => {
    console.error('Failed to fetch /value/wlanchan:', err);
  });
